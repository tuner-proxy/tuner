      ...
      <div class={{ [style.expanded]: expanded.value }}>
        ...
        >
          <span>
            ...
            <span title={title.value}>{renderPreview()}</span>
          </span>
        </div>
        {buildChildren.value && (
...
function NullValue() {
  return <span class={style.nullValue}>null</span>;
}

function NumberValue({ value }: { value: number }) {
  return <span class={style.numberValue}>{value}</span>;
}

function BooleanValue({ value }: { value: boolean }) {
  return <span class={style.numberValue}>{String(value)}</span>;
}

function StringValue({ value }: { value: string }) {
  const normalizedValue = JSON.stringify(value);
  return <span class={style.stringValue}>{normalizedValue}</span>;
}
...
function ObjectInnerPreview(_props: { value: any }) {
  return <>{'{…}'}</>;
}
